...
/// The style 'auto' stands for on this platform. Read from the user agent
/// rather than asked of Rust: it is synchronous, so the first paint already
/// carries the right style instead of flashing the default one first. Every
/// webview Tauri uses names its OS there — WebView2 says "Windows",
/// WebKitGTK says "Linux" (or "X11").
function platformStyle(userAgent = '') {
  if (/Windows/i.test(userAgent)) return 'windows';
  if (/Linux|X11/i.test(userAgent) && !/Android/i.test(userAgent)) return 'gnome';
  return 'ios';
}
...
export function resolveUiStyle(preference, userAgent) {
  ...
  return platformStyle(userAgent);
}
