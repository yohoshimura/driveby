...
> **Note on layout:** the app now lives at the repository root. It used to sit under a per-release version folder (`v1.4.0/`, and before that one folder per release); those folders were flattened away. Earlier releases (`v1.0.0`–`v1.3.0` and the `v0.x-beta` pre-Tauri snapshots) remain in git history at commit `a768e06` and can be recovered with `git checkout a768e06 -- <version-folder>`.
