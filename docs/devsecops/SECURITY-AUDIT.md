...
|---|---|---|---|
| Critical | On Linux/macOS a `\` in a file name became `/` in the relative path, so a source or backup entry named `..\..\x` was written **outside the destination** by backup and by restore. | The backup and restore walks skip such names (`backup::has_foreign_separator`). Tests fail without the fix. | 1-2-1, 2-1-1 |
...
| Medium | `start_backup` trusts the task the webview sends; pointed at the home folder a mirror would prune every document. | Refuse the home folder and its ancestors as a destination. (See *Not fixed* for the rest.) | 2-1-2, 3-2 |
